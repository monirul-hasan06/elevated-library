...
export async function POST(req: Request) {
  try {
    ...
    // Check current site mode.
    // If site is in guest mode, authenticated checkout should not be accepted.
    ...
    if (
      settings?.site_mode === "guest" &&
      input.checkoutType === "authenticated"
    ) {
      return NextResponse.json(
        { error: "Login checkout is disabled. Please use guest checkout." },
        { status: 403 }
      );
    }
    ...
    // Guest validation must be outside authenticated block.
    if (input.checkoutType === "guest") {
      if (!email) {
        return NextResponse.json(
    ...
    const [{ data: product, error: productError }, { data: method, error: methodError }] =
      await Promise.all([
        admin
          .from("products")
          .select("*")
    ...
    const downloadUrl = rawToken
      ? `${getSiteUrl()}/download?token=${encodeURIComponent(rawToken)}`
      : undefined;
    ...
    if (error) {
      if (error.code === "23505") {
        return NextResponse.json(
          { error: "This TrxID already submitted" },
          { status: 409 }
        );
      }
    ...
    await Promise.allSettled([
      ...
      notifyAdmin({
        ...
        html: `
          <p>New pending order: ${order.id}</p>
          <p>Customer: ${input.checkoutType === "guest" ? input.guestName : email}</p>
          <p>Product: ${product.title_en || product.title_bn}</p>
          <p>Amount: ${money(order.amount)}</p>
          <p>Payment Method: ${method.label || method.method}</p>
          <p>TrxID: ${order.trx_id}</p>

...
export function NoticeBar({ notice }: { notice?: any }) {
  ...
  const hasBnMessage = Boolean(notice.message_bn?.trim());
  const hasEnMessage = Boolean(notice.message_en?.trim());
  const hasMessage = hasBnMessage || hasEnMessage;
  ...
  return (
    <div className="bg-brand-600 px-4 py-2 text-sm text-white">
      <div className="container-page flex items-center justify-between gap-3">
        <p>
          <strong>
            <T bn={notice.title_bn} en={notice.title_en} />
          </strong>

          {hasMessage ? (
            <span>
              : <T bn={notice.message_bn} en={notice.message_en} />
            </span>
          ) : null}
        </p>
